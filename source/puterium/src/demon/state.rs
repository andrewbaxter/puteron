...
pub(crate) struct TaskStatePerpetual {
    ...
    pub(crate) spec: interface::task::TaskSpecPerpetual,
}
...
pub(crate) enum TaskStateSpecific {
    ...
    Perpetual(TaskStatePerpetual),
    Finite(TaskStateFinite),
    External,
}
...
>(t: &'a TaskState_, mut cb: impl FnMut(&mut dyn Iterator<Item = (&'a TaskId, &'a DependencyInfo)>) -> T) -> T {
    match &t.specific {
        ...
        TaskStateSpecific::Finite(s) => return cb(&mut s.spec.upstream.iter()),
